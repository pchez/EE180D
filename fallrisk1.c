//compile using gcc -lmraa -lm -o fallrisk1 fallrisk1.c LSM9DS0.c
#include <stdio.h>
#include <sys/types.h>
...
int fallTrain()
{
	...
	fann_train_on_file(ann, "fall.data", max_epochs, epochs_between_reports, desired_error);

	fann_save(ann, "fall.net");
...
int fallTest(int sequence_number, float rotation_degrees)
{
	fann_type *calc_out;
	fann_type input[3];

	struct fann *ann = fann_create_from_file("fall.net");
	...
	while(1) {
		...
		if (count == 3) {
           ...
           if (isMoving(gyro_data)==0)        //if patient is stationary, calculate new posture
            ...
            //////find posture for the current interval//////
			int posture_buffer[2]; //replace with queue
			time_curr = time(NULL);
			time_elapsed = time_curr - time_prev;
			if (time_elapsed >= 5.0)	//each interval is 5 seconds
			{
				//if queue is full
					//add current posture to queue and remove the oldest item from queue
					//add switch statement here to use queue values to assign the sequence #
					//fallTest(sequence#, rotationdegrees); this passes sequence number and rotation to neural network
				//if queue not full, add current posture to queue
				time_prev = time_curr;
